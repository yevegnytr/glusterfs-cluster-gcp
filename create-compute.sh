...
for (( i=1; i<=${COUNT}; i++ ))
  do
    ...
    --zone=${GCP_REGION}-${GCP_AZS[$i-1]} --machine-type=$GFS_NODE_TYPE --subnet=$GCP_SUBNET --network-tier=PREMIUM --maintenance-policy=MIGRATE \
    --service-account=$GCP_SVC_ACC --scopes=$GCP_API_SCOPE --tags=$GCP_FW_TAG\
    --image=$GCP_IMAGE --image-project=$GCP_IMAGE_PRJ --boot-disk-size=$GCP_BOOT_DISK_SIZE --boot-disk-type=$GCP_BOOT_DISK_TYPE \
